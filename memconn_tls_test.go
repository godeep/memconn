...
func TestTLS_MemConn(t *testing.T) {
	testTLS(t, "memu", "localhost", true)
}

func TestTLS_MemConn_NoTLS(t *testing.T) {
...
func TestTLS_HTTP_MemConn(t *testing.T) {
	var (
		rootCAs     *x509.CertPool
		hostKeyPair tls.Certificate
	)
	...
	client := &http.Client{
		Transport: &http.Transport{
			DialContext: func(
				...
				// in TLS.
				conn, err := memconn.DialContext(ctx, "memu", "MyNamedNetwork")
				if err != nil {
					return nil, err
				}
...
func (c *connLogger) logReadOrWrite(op string, n int, b []byte) func(error) {
	...
	switch op {
	case "R1":
		id = atomic.AddInt64(c.opCounter, 1)
	case "R2":
		id = atomic.LoadInt64(c.opCounter)
	case "W1":
		id = atomic.AddInt64(c.opCounter, 1)
	case "W2":
		id = atomic.LoadInt64(c.opCounter)
